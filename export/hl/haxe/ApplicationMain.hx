...
class ApplicationMain
{
	...
	macro public static function getEntryPoint()
	{
		...
		switch (Context.follow(Context.getType("funkin.Main")))
		{
			case TInst(t, params):
				...
				if (hasMain)
				{
					return Context.parse("@:privateAccess funkin.Main.main()", Context.currentPos());
				}
				...
				{
					Context.fatalError("Main class \"funkin.Main\" has neither a static main nor a constructor.", Context.currentPos());
				}

			default:

				Context.fatalError("Main class \"funkin.Main\" isn't a class.", Context.currentPos());
		}
...
@:keep @:dox(hide) class DocumentClass extends funkin.Main {}
#else
class DocumentClass
{
